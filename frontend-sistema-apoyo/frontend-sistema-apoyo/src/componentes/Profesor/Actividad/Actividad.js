      ...
      <div className="flex-grow flex flex-col items-center justify-center px-5 py-10">
        ...
        <div className="w-full max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6">
          {loading ? (
            ...
            filteredActividades.map((actividad) => (
              <div key={actividad.idactividad} className="flex flex-col items-center justify-between bg-white rounded-lg shadow-md overflow-hidden transition-transform duration-300 hover:scale-105">
                <img src={actividadImg} alt="actividad" className="w-full h-40 object-cover" />
                <div className="p-4">
                  <h3 className="font-semibold text-xl text-gray-800 truncate">{actividad.nombre || "Sin título"}</h3>
                  <p className="text-gray-600 text-sm">{actividad.descripcion || "Sin descripción"}</p>
                  <button
                    onClick={() => handleViewActividad(actividad)}
