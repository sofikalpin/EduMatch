...
export const Registrar = () => {
  ...
  // Estado para guardar únicamente los correos existentes
  ...
  // Obtenemos la lista de usuarios y extraemos solo los correos
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        let usersArray = data.users ? data.users : data;
        if (!Array.isArray(usersArray)) {
          usersArray = [usersArray];
        }
        // Extraemos los correos tal como vienen en la respuesta
  ...
  const validateEmail = async () => {
    ...
    try {
      // Se compara el correo ingresado con los existentes tal como vienen
  ...
  const validateStep = async () => {
    ...
    if (step === 3) {
      // En el paso 3 se valida el correo
  ...
  const handleSubmit = async (e) => {
    ...
    if (isValid) {
      ...
      try {
        await fetch("http://localhost:5228/API/Usuario/GuardarUsuario", {
          method: "POST",
          headers: {
        ...
        if (formData.userType === "profesor") {
          navigate("/subirCV"); // Redirige a la pantalla de subir CV
        } else {
